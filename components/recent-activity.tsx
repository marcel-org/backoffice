...
export function RecentActivity() {
  ...
  return (
    <Card className="overflow-hidden rounded-[1.75rem] border border-white/10 bg-[linear-gradient(180deg,rgba(255,255,255,0.08),rgba(255,255,255,0.03))] shadow-[0_24px_60px_rgba(0,0,0,0.22)] backdrop-blur-xl">
      <CardHeader className="border-b border-white/10">
        <p className="text-xs uppercase tracking-[0.22em] text-stone-500">
          Operations
        </p>
        <CardTitle className="flex items-center gap-2 text-xl font-semibold tracking-[-0.03em] text-stone-100">
          ...
          {displayedActivities?.length === 0 ? (
            ...
            displayedActivities?.map((activity) => {
              ...
              return (
                <div
                  key={activity.id}
                  className="flex items-start gap-4 px-6 py-5 transition-colors duration-300 hover:bg-white/5"
                >
                  <div
        ...
        {activities && activities.length > 10 && (
          <div className="border-t border-white/10 bg-black/10 px-6 py-4">
            <Button
              variant="ghost"
              size="sm"
              className="rounded-full border border-white/10 bg-white/5 px-4 text-sm text-stone-200 transition-all duration-300 hover:border-[hsl(var(--primary))/0.4] hover:bg-white/10 hover:text-[hsl(var(--primary))]"
